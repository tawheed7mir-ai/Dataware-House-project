/********************************************************************************************
 ...
 🧠 Next Step:
   Data from Silver → Gold Layer for analytics, reporting, and BI dashboards.
...
-- Step 2️⃣: Create the Silver table for cleaned Product Info
...
-- Step 3️⃣: Transform and Insert data from Bronze → Silver
...
SELECT 
    ...
    -- 🏷️ Map product line codes to readable names
    ...
    -- 🗓️ Convert product start date
    CAST(prd_start_dt AS DATE) AS prd_start_dt,

    -- 🗓️ Derive product end date using LEAD function
...
-- Step 3️⃣: Transform and insert clean data from Bronze layer
...
SELECT 
    ...
    -- ✅ Clean & Convert Order Date
    CASE  
        WHEN sls_order_dt = 0 OR LENGTH(sls_order_dt) != 8 THEN NULL
        ELSE STR_TO_DATE(CAST(sls_order_dt AS CHAR), '%Y%m%d')
    END AS sls_order_dt,

    -- ✅ Clean & Convert Ship Date
    CASE  
        WHEN sls_ship_dt = 0 OR LENGTH(sls_ship_dt) != 8 THEN NULL
        ELSE STR_TO_DATE(CAST(sls_ship_dt AS CHAR), '%Y%m%d')
    END AS sls_ship_dt,

    -- ✅ Clean & Convert Due Date
    CASE  
    ...
    -- ✅ Fix invalid or negative prices
      CASE 
...
/******************************************************************************************
🥈 SILVER LAYER - ERP LOCATION DATA
AUTHOR: Tawheed Mir
PURPOSE:
...
-- Step 1️⃣: Create Silver table
CREATE TABLE IF NOT EXISTS erp_loc_a101 (
    cid VARCHAR(50),
    cntry VARCHAR(50)
);

-- Step 2️⃣: Transform and insert clean data
...
/******************************************************************************************
🥈 SILVER LAYER - ERP CUSTOMER DATA
AUTHOR: Tawheed Mir
PURPOSE:
    Transform ERP customer data from the Bronze layer into the Silver layer.
    Cleans invalid values, standardizes formats, and ensures consistency.
...
-- Step 1️⃣: Create Silver table for ERP Customer Data
...
-- Step 2️⃣: Transform and insert data from Bronze → Silver
...
/******************************************************************************************
🥈 SILVER LAYER - ERP PRODUCT CATEGORY DATA
AUTHOR: Tawheed Mir
PURPOSE:
    Promote ERP product category and maintenance data from the Bronze layer
    to the Silver layer without transformation, as data quality is already sufficient.

NOTES:
    - No transformation required; structure and values are standardized.
    - Table used for joining with other ERP or CRM product details in Silver/Gold layers.
******************************************************************************************/



-- Step 1️⃣: Create Silver table for ERP Product Category Data
CREATE TABLE IF NOT EXISTS silver.erp_px_ct_g1v2 (
    id VARCHAR(50),
    cat VARCHAR(50),
    subcat VARCHAR(50),
...
-- Step 2️⃣: Directly insert clean data from Bronze → Silver
INSERT INTO silver.erp_px_ct_g1v2 (id, cat, subcat, maintenance)
SELECT 
    id,
    cat,
    subcat,
    maintenance
FROM bronze.erp_px_ct_g1v2;
